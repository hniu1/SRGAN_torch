...
from srgan_torch import SRGAN_g, SRGAN_d, SRGAN_g_lr, SRGAN_d_lr, SRGAN_g_lr_upsample
...
version = 'v0.72' # check the version.txt file for historical versions under output directory
save_dir = "samples"
elevation = False
elevation_hr = False
...
if args.mode == 'train' and readrawdata:
    train_lr, test_lr, train_hr, test_hr = daymetread(path_output, checkpoint_dir, elevation, elevation_hr, high_deg=False, scaler = 'minmax')
    # Load the scaler
...
# Initialize models
G = SRGAN_g_lr_upsample().to(device)
D = SRGAN_d_lr(hr_size=train_hr[0].shape[0]*train_hr[0].shape[1]).to(device)
