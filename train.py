...
from srgan_torch import SRGAN_g, SRGAN_d, SRGAN_g_lr, SRGAN_d_lr
...
# Initialize models
G = SRGAN_g().to(device)
D = SRGAN_d(hr_size=train_hr[0].shape[0]*train_hr[0].shape[1]).to(device)
...
def train():
    ...
    # Define optimizers
    g_optimizer_init = optim.Adam(G.parameters(), lr=1e-4)
    g_optimizer = optim.Adam(G.parameters(), lr=1e-4)
    d_optimizer = optim.Adam(D.parameters(), lr=1e-4)

    # Define learning rate schedulers
    g_lr_scheduler_init = torch.optim.lr_scheduler.StepLR(g_optimizer_init, step_size=100, gamma=0.5)
    g_lr_scheduler = torch.optim.lr_scheduler.StepLR(g_optimizer, step_size=500, gamma=0.5)
    d_lr_scheduler = torch.optim.lr_scheduler.StepLR(d_optimizer, step_size=500, gamma=0.5)
    ...
    if initial_training:
        ...
        min_delta_init = 0.00001    # Minimum change to qualify as an improvement during init phase
        ...
        for epoch in range(n_epoch_init):
            ...
            print(f"Epoch [{epoch+1}/{n_epoch_init}], Avg G Loss: {g_loss_avg:.5f}")
            ...
            if g_loss_avg < best_loss_init - min_delta_init:
                ...
                print(f"New best model saved with G Loss: {best_loss_init:.5f}")
            else:
    ...
    min_delta_adv = 0.00001      # Minimum change to qualify as an improvement during adversarial phase
    ...
    for epoch in range(n_epoch):
        ...
        print(f"Epoch: [{epoch+1}/{n_epoch}], Avg G Loss: {g_loss:.5f}, Avg D Loss: {d_loss:.5f}")
        ...
        if g_loss < best_g_loss_adv - min_delta_adv:
            ...
            print(f"New best G model saved with G Loss: {best_g_loss_adv:.5f}")
