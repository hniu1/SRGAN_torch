...
description: Run or prepare REFINE multivariable inference for tmin, tmax, and precipitation at 1 degree to 0.25 degree, 0.25 degree to 1/24 degree, or as a two-stage cascade. Use for input validation, checkpoint selection, inference commands, and output interpretation. Do not use for model training, scheduler operations, or quantitative evaluation.
---

# REFINE Downscaling

Use the repository's trained REFINE models to downscale aligned daily environmental fields. Treat the three variables as one model input; accept separate source files so new variables remain composable.

## Workflow

1. Locate the repository root. It contains `pipeline_04_downscale_stage1.py`.
...
3. Read [references/data-contract.md](references/data-contract.md) and validate every input before inference:
   ...
   ```bash
   python skills/refine-downscaling/scripts/validate_inputs.py \
     --stage stage1 \
...
- Do not submit scheduler jobs or overwrite existing outputs without explicit authorization. Use `$refine-ops` for Slurm operations.
- Use `$refine-evaluation` to calculate metrics or spatial statistics.
