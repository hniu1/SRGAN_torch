...
def read_saved_data(name, path_output, scaler):
    arr = np.load(f'{path_output}/{name}.npy')
    shp = arr.shape
    flat = arr.reshape(-1, 1)
    scaled = scaler.transform(flat).reshape(shp[0], shp[1], shp[2], 1)
    return scaled
...
def read_Daymet_yearly(var, year_start, year_end, deg=1, Daymet_ERA5=True, yearly=True):
    arrays = []
    if yearly:
        for year in range(year_start, year_end): #2023
            ...
            if Daymet_ERA5:
                ...
                try:
                    fil        = Dataset(f'/lustre/orion/proj-shared/cli138/dr6/NA-Downscaling/data/Daymet_ERA5_{var}_dy_{year}_{deg}deg.nc')
                except:
                    if deg == 0.25:
                        deg_str = '0p25'
                        fil        = Dataset(f'/lustre/orion/proj-shared/cli138/dr6/NA-Downscaling/data/Daymet_ERA5_{var}_dy_{year}_{deg_str}deg.nc')
                    elif deg == 0.0416:
                        deg_str = 'trim'
                        fil        = Dataset(f'/lustre/orion/proj-shared/cli138/dr6/NA-Downscaling/data/Daymet_ERA5_{var}_dy_{year}_{deg_str}.nc')
            else:
                fil        = Dataset(f'/lustre/orion/cli138/proj-shared/7hn/data/Daymet/DaymetV4_VIC4_{var}_{year}_{deg}deg_US.nc')
            hr_var     = fil.variables[f'{var}_dy'][:]
        ...
        for year in range(year_start, year_end): #2023
            for month in range(1,13):
                if month < 10:
                    ...
                    month = str(month)
                print(f'read {deg} degree data from year {year} month {month}')
                if Daymet_ERA5:
                    fil        = Dataset(f'/lustre/orion/cli138/proj-shared/dr6/NA-Downscaling/data/Daymet_ERA5_{var}_3h_{year}{month}_{deg}deg.nc')
                else:
                    fil        = Dataset(f'/lustre/orion/cli138/proj-shared/7hn/data/Daymet/DaymetV4_VIC4_{var}_{year}_{deg}deg_US.nc')
                hr_var     = fil.variables[f'{var}_3h'][:]
                arrays.append(np.array(hr_var))
...
def read_elev(tt, deg=1):
    felev      = Dataset(f'/lustre/orion/proj-shared/cli138/dr6/NA-Downscaling/DEM/final-elev/VICa_DEM_{deg}deg_fill0.nc')
...
# Read variables nd generate low resolution version
    ...
    if not Daymet_ERA5:
        lr_prect = read_Daymet_yearly(var, year_start=year_start, year_end=year_end,deg=deg_lr, yearly=yearly) #1990
        hr_prect = read_Daymet_yearly(var, year_start=year_start, year_end=year_end, deg=deg_hr, yearly=yearly)
    else:
        lr_prect = read_Daymet_yearly(var, year_start=year_start, year_end=year_end, deg=deg_lr, Daymet_ERA5=Daymet_ERA5, yearly=yearly) #1990
        hr_prect = read_Daymet_yearly(var, year_start=year_start, year_end=year_end, deg=deg_hr, Daymet_ERA5=Daymet_ERA5, yearly=yearly)
