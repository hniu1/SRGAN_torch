...
class GCM_Downscaling:
    ...
    def downscale_100_25(self):
        out_path = f"{self.path_downscaling}/y_pred_25.npy"
        if os.path.exists(out_path):
            print(f"Skip 100->25 km: output already exists at {out_path}")
            return
        ...
        np.save(out_path, yinv)
        print("Downscaling from 100 to 25 km finished.")


    def downscale_25_4(self):
        out_path = f'{self.path_downscaling}/y_pred_4.npy'
        if os.path.exists(out_path):
            print(f"Skip 25->4 km: output already exists at {out_path}")
            return
        ...
        with torch.no_grad():
            for i in tqdm(
                ...
                # NOTE: must use loaded_scaler_hr here — the HR model input was
                # normalised with loaded_scaler_hr, so the inverse must match.
                flat = pred.reshape(-1, 1)
                inv = self.inverse_scale_chunked(
                    flat,
                    self.loaded_scaler_hr,
...
if __name__ == '__main__':
    version = '0.1_test_param'
    
    if version == '0.1_test_param':
        year = '198001-201912'
    elif version == '0.2':
        year = '202001-205912'
    ...
    for var in ['pr', 'tmax', 'tmin']:
        ...
        gcms = ['ACCESS-CM2', 'BCC-CSM2-MR', 'CNRM-ESM2-1', 'EC-Earth3-CC', 'EC-Earth3-Veg', 'GFDL-CM4', 'MRI-ESM2-0', 'MPI-ESM1-2-HR', 'NorESM2-MM', 'TaiESM1'][1:]
        ...
        for gcm in gcms:
            ...
            path_gcm = f'/lustre/orion/proj-shared/cli138/dr6/NA-Downscaling/gcm/{var}_day_{gcm}_{scenario}_{ens}_{grid}_{year}_1deg_NA_BC.nc'
            
            if not os.path.exists(path_gcm):
                print(f'GCM data not found at {path_gcm}, skipping {gcm}...')
                continue
            ...
            out_25 = f'./gcm_ds/{version}/{var}/{gcm}/y_pred_25.npy'
            out_4 = f'./gcm_ds/{version}/{var}/{gcm}/y_pred_4.npy'

            if os.path.exists(out_4):
                print(f'Skip {var} {scenario} {gcm}: 4 km output already exists at {out_4}')
                continue

            if os.path.exists(out_25):
                print(f'Skip 100->25 km for {var} {scenario} {gcm}: output exists at {out_25}')
            else:
